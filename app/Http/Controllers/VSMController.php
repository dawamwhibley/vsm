...
//        Log::info('COS SIMILARITY ' . json_encode($cos_similarity));
    ...
    public static function dokumen_term($dokumen)
    {
        ...
        foreach ($dokumen as $key => $value) {
            ...
            $term       = array_count_values($word);
            array_push($arrayTampung, ['id_doc' => $value['id_doc'], 'judul' => $value['judul'], 'dokumen' => $term]);
        }
        return $arrayTampung;
    }
    ...
    public static function bobot($query, $dokumen_term, $idf)
    {
        ...
        foreach ($dokumen_term as $index => $dokumen) {
            ...
            array_push($bobotDokumen, array('id_doc' => $dokumen['id_doc'], 'judul' => $dokumen['judul'], "dokumen" => $arrayTampung));
        }
    ...
    public static function cosine_similarity($bobot)
    {
        ...
        foreach ($bobot['dokumen'] as $index => $dokumen) { // dokumen 1, 2, 3 ... n
            $arrayDoc[$index] = ["id_doc" => $dokumen['id_doc'], "judul" => $dokumen['judul']];
            $bobotDoc[$index] = 0;
            foreach ($bobot['query'] as $key => $value) {
        ...
        foreach ($dokumenVektor as $index => $dokumen) {
            $arrayDoc[$index] = ["id_doc" => $dokumen['id_doc'], "judul" => $dokumen['judul'], "jumlah_vektor" => $dokumen['jumlah_vektor'] ];
        ...
        foreach ($dokumenCosine as $index => $dokumen) {
            ...
            $arrayDoc[$index] = ["id_doc" => $dokumen['id_doc'], "judul" => $dokumen['judul'], "ranking" => $jumlah];
